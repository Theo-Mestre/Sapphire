...
namespace sph
{
	...
	static void SerializeEntity(YAML::Emitter& out, Entity entity)
	{
		...
		if (entity.HasComponent<ParallaxComponent>())
		{
			out << YAML::Key << "ParallaxComponent";
			out << YAML::BeginMap;

			auto& nativeScriptComponent = entity.GetComponent<ParallaxComponent>();
			out << YAML::Key << "Speed" << YAML::Value << nativeScriptComponent.Speed;
			out << YAML::Key << "InitialPosition" << YAML::Value << nativeScriptComponent.InitialPosition;
			out << YAML::Key << "Threshold" << YAML::Value << nativeScriptComponent.Threshold;
	...
	bool SceneSerializer::Deserialize(const std::string& _filepath)
	{
		...
		for (auto entity : entities)
		{
			...
			auto parallaxComponent = entity["ParallaxComponent"];
			if (parallaxComponent)
			{
				auto& pc = deserializedEntity.AddComponent<ParallaxComponent>();
				pc.Speed = parallaxComponent["Speed"].as<glm::vec2>();
